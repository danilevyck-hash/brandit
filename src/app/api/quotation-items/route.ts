...
export async function POST(request: NextRequest) {
  ...
  const { data, error } = await getSupabaseServer().from("quotation_items").insert(items).select();
...
export async function PUT(request: NextRequest) {
  ...
  const { data, error } = await getSupabaseServer().from("quotation_items").update(updates).eq("id", id).select().single();
...
export async function DELETE(request: NextRequest) {
  ...
  const { error } = await getSupabaseServer().from("quotation_items").delete().eq("id", id);
