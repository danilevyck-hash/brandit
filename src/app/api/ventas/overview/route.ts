// GET /api/ventas/overview?year&mes → RPC ventas_overview_v1
...
export async function GET(req: NextRequest) {
  ...
  const { data, error } = await getSupabaseServer().rpc("ventas_overview_v1", { p_year: year, p_mes: mes });
  if (error) {
    console.error("[api/ventas/overview] rpc error", error);
