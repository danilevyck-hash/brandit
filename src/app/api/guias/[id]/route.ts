...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const { fecha, modo_entrega, transportista_id, placa, observaciones, items, monto_total, estado, receptor_nombre, cedula, firma_base64, firma_entregador_base64, entregado_por, numero_guia_transp, tipo_despacho, nombre_chofer, motivo_rechazo } = body;
  ...
  if (modo_entrega !== undefined) {
    ...
    if (modo_entrega === "transportista" && !transportista_id) return NextResponse.json({ error: "Selecciona un transportista" }, { status: 400 });
  }
  ...
  if (modo_entrega !== undefined) { u.modo_entrega = modo_entrega; u.transportista_id = modo_entrega === "transportista" ? transportista_id : null; }
