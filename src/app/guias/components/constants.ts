...
// Brand It es mono-empresa: NO hay catálogo de empresas.
// Las listas de clientes, direcciones y entregadores arrancan vacías —
// Daniel las carga desde el form (se persisten en localStorage vía loadList/saveList).

export const DEFAULT_CLIENTES: string[] = [];
export const DEFAULT_DIRECCIONES: string[] = [];
export const DEFAULT_ENTREGADORES: string[] = [];
...
export function emptyItem(orden: number): GuiaItem {
  return { orden, cliente: "", direccion: "", facturas: "", bultos: 0, numero_guia_transp: "" };
}

export function clientesSummary(items: GuiaItem[]): string {
  if (!items || items.length === 0) return "";
  const uniqueClientes = Array.from(new Set(items.map((i) => i.cliente).filter(Boolean)));
